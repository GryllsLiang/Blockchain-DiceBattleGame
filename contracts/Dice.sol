...
contract Dice {
    ...
    function add(
        ...
        require(numberOfSides > 0, "dice must have at least 1 side");
    ...
    modifier ownerOnly(uint256 diceId) {
        require(dices[diceId].owner == msg.sender, "Dice can only be transferred by owner");
        _;
    }
    
    modifier validDiceId(uint256 diceId) {
        require(diceId < numDices, "dice ID must be less than number of dices");
        _;
    }
